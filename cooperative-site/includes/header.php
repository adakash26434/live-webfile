...
/* ── Cache: pages table has show_in_menu column? (1 query for all 3 menu sections) ── */
$_navPagesMenuColExists = false;
$_navAboutMenuPages     = [];
$_navServicesMenuPages  = [];
$_navMoreMenuPages      = [];
try {
    if ($db) {
        $checkCol = $db->query("SHOW COLUMNS FROM pages LIKE 'show_in_menu'");
        $_navPagesMenuColExists = ($checkCol && $checkCol->fetch() !== false);
        if ($_navPagesMenuColExists) {
            $allMenuPages = $db->query(
                "SELECT id, slug, title, COALESCE(title_en,'') AS title_en,
                        COALESCE(is_new,0) AS is_new, new_until, menu_position
                 FROM pages
                 WHERE is_active = 1 AND show_in_menu = 1
                 AND menu_position IN ('about','services','more')
                 ORDER BY menu_position, menu_order ASC LIMIT 15"
            )->fetchAll();
            foreach ($allMenuPages as $mp) {
                switch ($mp['menu_position']) {
                    case 'about':    $_navAboutMenuPages[]    = $mp; break;
                    case 'services': $_navServicesMenuPages[] = $mp; break;
                    case 'more':     $_navMoreMenuPages[]     = $mp; break;
                }
            }
        }
    }
...
/* Helper — render a dynamic page nav item */
$_navPageItem = static function (array $mp, string $lang, string $siteUrl): string {
    $title = ($lang === 'en' && !empty($mp['title_en'])) ? $mp['title_en'] : ($mp['title'] ?: $mp['title_en']);
    $isNew = !empty($mp['is_new']) && (!$mp['new_until'] || strtotime($mp['new_until']) >= time());
    $newBadge = $isNew ? ('<span class="nav-new-badge">' . ($lang === 'en' ? 'New' : 'नयाँ') . '</span>') : '';
    return '<li><a href="' . htmlspecialchars($siteUrl . 'page.php?slug=' . urlencode($mp['slug']), ENT_QUOTES, 'UTF-8') . '">'
        . htmlspecialchars($title, ENT_QUOTES, 'UTF-8') . $newBadge . '</a></li>';
};
...
<head>
    ...
    <!-- Prevent flash of wrong theme — apply dark mode immediately if user preference is dark -->
    <script>(function(){var s=localStorage.getItem('coop_dark_mode');if(s==='1'||(s===null&&window.matchMedia&&window.matchMedia('(prefers-color-scheme: dark)').matches)){document.documentElement.classList.add('dark-mode-preload');}})();</script>
...
<body class="header-v2<?php echo !empty($__embed_frame) ? ' embed-in-member-portal' : ''; ?>">
                        ...
                        <ul class="nav-menu">
                            ...
                            <li class="has-dropdown <?php echo $currentPage == 'about' ? 'active' : ''; ?>">
                                ...
                                <ul class="dropdown">
                                    ...
                                    <li><a href="<?php echo SITE_URL; ?>institutional-profile.php"><i class="fas fa-building-columns text-success me-1"></i> <?php echo isEnglish() ? 'Institutional Profile' : 'संस्थागत प्रोफाइल'; ?></a></li>
                                    <?php
                                    foreach ($_navAboutMenuPages as $mp) {
                                        echo $_navPageItem($mp, $currentLang, SITE_URL);
                                    }
                                    ?>
                                </ul>
                            </li>
                            <li class="has-dropdown <?php echo $currentPage == 'services' ? 'active' : ''; ?>">
                                ...
                                <ul class="dropdown">
                                    ...
                                    <?php endif; ?>
                                    <?php
                                    foreach ($_navServicesMenuPages as $mp) {
                                        echo $_navPageItem($mp, $currentLang, SITE_URL);
                                    }
                                    ?>
                                </ul>
                            ...
                            <li class="has-dropdown">
                                ...
                                <ul class="dropdown">
                                    ...
                                    <li><a href="<?php echo SITE_URL; ?>application-tracker.php"><i class="fas fa-search"></i> <?php echo isEnglish() ? 'Track Application' : 'आवेदन ट्र्याक'; ?></a></li>
                                    <?php
                                    foreach ($_navMoreMenuPages as $mp) {
                                        echo $_navPageItem($mp, $currentLang, SITE_URL);
                                    }
                                    ?>
                                </ul>
    ...
    @media (max-width: 991.98px) {
        body.header-v2 #mainNavV2.main-nav {
            ...
            background: var(--primary-dark, #0b3f24) !important;
            color: #fff !important;
