...
function getSetting($key, $default = '') {
    static $__cache = [];
    static $__all_loaded = false;

    // Return from per-key cache if already fetched this request
    if (array_key_exists($key, $__cache)) {
        return $__cache[$key] !== null ? $__cache[$key] : $default;
    }

    // Bulk-load all settings on first miss — eliminates N individual DB queries
    if (!$__all_loaded) {
        $__all_loaded = true;
        try {
            $db = getDB();
            ...
            foreach ($rows as $row) {
                if (!array_key_exists($row['setting_key'], $__cache)) {
                    $__cache[$row['setting_key']] = $row['setting_value'];
                }
            }
        } catch (\Throwable $e) { /* silent — single-key fallback below */ }

        // Return from cache after bulk load
        if (array_key_exists($key, $__cache)) {
            return $__cache[$key] !== null ? $__cache[$key] : $default;
        }
    }

    // Fallback: single-key query (first request if bulk failed, or key truly absent)
    try {
        $db = getDB();
...
if (!headers_sent()) {
    ...
    unset($_httpsOn);
    /* CSP — enforce mode (all CDN origins validated 2026-06-12) */
    header(
...
/**
 * =====================================================
 * LEGACY COMPATIBILITY: $conn (mysqli) — null placeholder
 * 2026-06-12 audit: no mysqli usage found in any PHP file.
 * Connection creation removed to save 1 DB connection per request.
 * =====================================================
 */
$conn = null; // backward-compat shim — do not use
