...
try {
    /* ── Batch employee status counts — 5 queries → 1 ── */
    $hRow = $db->query(
        "SELECT COUNT(*) AS total,
                SUM(status='active')     AS active,
                SUM(status='probation')  AS probation,
                SUM(status='on_leave')   AS on_leave,
                SUM(status IN ('resigned','terminated','retired')) AS exited
         FROM hrm_employees"
    )->fetch();
    if ($hRow) {
        $total     = (int)$hRow['total'];
        $active    = (int)$hRow['active'];
        $probation = (int)$hRow['probation'];
        $onLeave   = (int)$hRow['on_leave'];
        $exited    = (int)$hRow['exited'];
    }
